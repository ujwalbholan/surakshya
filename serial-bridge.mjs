...
const rl = createInterface({ input: createReadStream(SERIAL_PORT), crlfDelay: Infinity });
let buffer = [];
let inBlock = false;
...
rl.on('line', (line) => {
  ...
  if (inBlock) {
    buffer.push(line);
    if (line.startsWith('GPS UTC') || (line.startsWith('Speed:') && line.includes('km/h') && !line.startsWith('GPS'))) {
      inBlock = false;
...
process.on('SIGINT', () => { mqttClient.end(true); process.exit(); });
process.on('SIGTERM', () => { mqttClient.end(true); process.exit(); });

console.log(`Serial bridge listening on ${SERIAL_PORT} → ${MQTT_URL} topic ${TOPIC}`);
